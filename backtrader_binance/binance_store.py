...
class BinanceStore(object):
    ...
    def getdata(self, timeframe_in_seconds, start_date=None):
        if not self._data:
            self._data = BinanceData(store=self, timeframe_in_seconds=timeframe_in_seconds, start_date=start_date)
        return self._data
